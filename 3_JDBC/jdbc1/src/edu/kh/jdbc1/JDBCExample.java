...
public class JDBCExample {
	public static void main(String[] args) {
		...
		// 		JDBC를 상속받아 구현할 클래스모음(ojdbc11.jar 라이브러리)
		//		->OracleDriber.class(JDBC드라이버)이용
		...
		// Connection을 통해
		// SQL문을 DB에 전달하여 실행하고
		// 생선된 결과(ResultSet, 성공한 행의 개수)를 반환(Java)하는데 사용되는 객체
		...
		try {
			...
			//			(패키지명 + 클래스명)
			//	-> () 안에 작성된 클래스의 객체를 반환
			// -> 메모리에 객체가 생성되고, JDBD 필요시 알아서 참조해서 사용
			// --> 생략해도 자동으로 메모리 로드가 진행됨(명시적으로 작성하는걸 권장)
			
			// 2. 연결 정보를 담은 Connetion을 생성
			// -> DriverManager 객체를 이용해서 Connection 객체를 만들어 얻어옴
			
			String type = "jdbc:oracle:thin:@"; //JDBD 드라이버의 종류
			...
			while(rs.next()) {
				...
				//			 다음 행이 잇을 경우 true 반환/ 없으면 false 반환
				
				// rs.get자료형("컬럼명")
				String empId = rs.getString("EMP_ID");
				//현재 행의 EMP_ID문자열 컬럼의 값을 얻어옴
				...
				// java.sql.Date, java.util.Date 둘 다 가능
			...
			//SQPException : DB 관련 최상위 예외 ( == 데이터베이스에 관련된 모든 예외)
			e.printStackTrace();
		}finally {
			// 4단계 : 사용한 JDBC 객체 자원반환(close() )
