...
public class StudentService {
	...
	//Java.utill.ArrayList : 배열형태 List(가장 대표적인 List 자식 클래스)
	
	//ArrayList() 기본 생성자 : 기본 크기 10 짜리 리스트 생성, 리스트 크기가 늘었자 줄었다 하기 때문에 큰 의미가 없음
	//ArrayList(용량) : 용량만큼의 리스트 생성, 너무 큰 값을 작성하면 메모리를 많이 소모함.
	
	//List studentList = new ArrayList();
	List<Student> studentList = new ArrayList<Student>(); //검색 조쇵에 효율적
	...
	public void ex() { 
		...
		//(반환형)
		//List.get(index i) : 리스트에서 i 번째 인덱스에 있는 객체(Object)를 반환
		//반환형이 Object == 몯느 객체를 반환할 수 있다.
	...
	public void selectAll() {
		...
		if(studentList.isEmpty()) { //studentList가 비어있으면 true 반환
			...
			return; // 반환형 없는(void) return 구문은 현재 메소드를 종료하고 호출한 곳으로 돌아감
					// 자동으로 else 구문을 쓴것같은 효과
			
		}
		/*
		for(int i=0; i<studentList.size(); i++) {
				System.out.println(studentList.get(i));
		// studentList 에서  i번째 인덱스 요소를 얻어와 풀력
		}
		*/
		
		//향상된 for문 (for each문)
		// - 컬렉션,배열의 모든 요소를 순차적으로 반복 접근할 수 있는 for문
		//(순차적 : 0번 인덱스부터 마지막요소까지 인덱스를 1 씩 증가)
		
		//[작성법]
		//for(컬렉션 or 배열에서 꺼낸 한개의 요소를 저장할 변수 : 컬렉션 명 or 배열 명) {}
		int index = 0;
	...
	public String updateStudent() throws InputMismatchException {
		...
		// ->반환값 Student == 변경전 Student 객체가 담겨있음
		...
		}else if( index < 0) { //2)입력된 숫자가 0보다 작은가?(음수x)//+ 문자열을 입력한 경우 -> throws로 예외처리
			return"음수는 입력할 수 없습니다.";
		}else if(index >= studentList.size()) {	//3)입력된 숫자가 studentList 범위 내 인덱스 번호인가?
